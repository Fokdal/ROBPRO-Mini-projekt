...
void setup() {
  lineSensors.initFiveSensors();                                                // Initialization of the sensors
  calibratorObj.lineSensorCalibrate();                                          // Calibrating the sensors
  while(!calibratorObj.lineCalibrator());                                       // Calibrates the ZUMO to the black line.
}

// Main loop function
void loop() {
  // Checks if the robot registers a black line. If not, it just continues to drive forward.
  if (calibratorObj.readLineSensor(2) || calibratorObj.readLineSensor(0) || calibratorObj.readLineSensor(4)) {
    motors.setSpeeds(0, 0);

  } else {
    calibratorObj.straightLineDriver(0);
  }
}
